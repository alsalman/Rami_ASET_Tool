...
public class joTextAnnotation extends joShape{
	...
	private String GetText(){
		if (ParentHandle.GetAnnotationString().compareTo("") ==0)
			return "No TAG yet!";
		else
			return "TAG: " + ParentHandle.GetAnnotationString();
	}
 ...
 //-		
 		cav.drawRect(screenRect.left       ,screenRect.top + screenRect.height()/48,screenRect.left+ screenRect.width(),screenRect.top+  screenRect.height()/9, paint);
 		...
 		cav.drawText(GetText(), screenRect.left +  screenRect.width()/16,screenRect.top + screenRect.height()/12, paint);
 //-
