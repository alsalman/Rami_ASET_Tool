...
public class joPath extends joShape{
	...
	public void UpdatePath(Boolean withSmoothing ){
		if (!withSmoothing ){
		//without smoothing 
			path.reset();
 ...
 //-
	...
	public boolean IsGeoPointInsideMyGeoPoints(GeoPoint pts){
		int i, j;
		boolean c = false;
		  for (i = 0, j = PointsCount()-1; i < PointsCount(); j = i++) {
		    if ( ((
		    		getPoint(i).GeoPTS.getLongitudeE6()
		    			<
		    		pts.getLongitudeE6()
		    			) != (
		    		getPoint(j).GeoPTS.getLongitudeE6()
		    			<
		    		pts.getLongitudeE6()
		    			)) &&(
					pts.getLatitudeE6() 
						> (
					getPoint(j).GeoPTS.getLatitudeE6()
						-
					getPoint(i).GeoPTS.getLatitudeE6()
						) * (
					pts.getLongitudeE6()
						-
					getPoint(i).GeoPTS.getLongitudeE6()
...
//	}
	...
	public void RefreshPixelsFromGeo(org.osmdroid.views.MapView.Projection prj){
		this.translateX  = this.translateY = 0;
		Point tempPoint = new Point();
		for(JoPoint x : this.pts){
			prj.toMapPixels(x.GeoPTS,tempPoint);
			x.x = tempPoint.x;
			x.y = tempPoint.y;
		}
		ct = GetCenterPoint();
	...
	public void RefreshGeoFromPixelsIncludingTranslation(org.osmdroid.views.MapView mView){
		org.osmdroid.views.MapView.Projection prj = mView.getProjection();
		float [] res;
		for(JoPoint x : this.pts){
			res = sharedSettings.FromMapPixelToPixel(
					 x.x + translateX
					 ,
					 x.y + translateY
					 
					 ,mView, prj.getScreenRect());
					 x.GeoPTS = (GeoPoint)prj.fromPixels(res[0],res[1]);			
		}
	}

	public  int [] GetMinBoundBoxGeo(){
		int [] tmp   =  {0,0,0,0};
		tmp[2] = tmp[0] = pts.get(0).GeoPTS.getLatitudeE6(); //MinX
		tmp[3] = tmp[1] = pts.get(0).GeoPTS.getLongitudeE6(); //MinY
		
		
		
		for (int i=0; i< pts.size() ;i++){
			if (tmp[0] < pts.get(i).GeoPTS.getLatitudeE6()  ) tmp[0] = pts.get(i).GeoPTS.getLatitudeE6();  //minX
			if (tmp[2] > pts.get(i).GeoPTS.getLatitudeE6()  ) tmp[2] = pts.get(i).GeoPTS.getLatitudeE6();  //MaxX
			
			if (tmp[1] > pts.get(i).GeoPTS.getLongitudeE6() ) tmp[1] = pts.get(i).GeoPTS.getLongitudeE6();  //minY
			if (tmp[3] < pts.get(i).GeoPTS.getLongitudeE6() ) tmp[3] = pts.get(i).GeoPTS.getLongitudeE6();  //minX
...
//		tmp[0] = pts.get(0).GeoPTS.getLatitudeE6();
//		tmp[1] = pts.get(0).GeoPTS.getLongitudeE6();
// 		
//		tmp[2] = pts.get(0).GeoPTS.getLatitudeE6();
//		tmp[3] = pts.get(0).GeoPTS.getLongitudeE6();
// 		
		return tmp;
